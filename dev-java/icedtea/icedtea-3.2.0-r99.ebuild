...
IUSE="+alsa cacao +cups doc examples +gtk headless-awt infinality
	jamvm +jbootstrap kerberos libressl nsplugin pax_kernel +pch X
...
COMMON_DEP="
	...
	!elibc_musl? ( >=dev-util/systemtap-1 )
...
DEPEND="${COMMON_DEP} ${ALSA_COMMON_DEP} ${CUPS_COMMON_DEP}
	X? ( ${X_COMMON_DEP} ${X_DEPEND} x11-libs/libXt )
	|| (
...
pkg_config() {
	einfo "emerge -1v --noreplace $(echo ${X_COMMON_DEP} ${X_DEPEND} | sed -e 's/:0\?=//g' -e 's/>/\\>/g') x11-libs/libXt"
}
